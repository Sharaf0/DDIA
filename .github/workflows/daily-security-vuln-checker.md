...
safe-outputs:
  create-issue:
    ...
    allowed-labels: [security, vulnerability, codeql, dependabot, critical, high, medium, low]
    max: 10
  ...
  add-comment:
    ...
    max: 10
  add-labels:
    allowed: [security, vulnerability, codeql, dependabot, critical, high, medium, low]
    target: "*"
    max: 50
  close-issue:
    target: "*"
    required-labels: [security, vulnerability]
    max: 20
network:
  allowed:
    - defaults
